...
M.run = scenes.skippable(function ()
	local mission_list = {
		{
			intl_key = "consult_explosive_forensics",
			id = "consult_explosive_forensics",
			intl_namespace = "chapter2",
			position = cp.mission_positions.consult_explosive_forensics,
			modifiers = {
				tab = -10
			}
		}
	...
	cp.add_informer_stimulants(budget_options, cp_index, 2)
	...
	if missions.completed.consult_explosive_forensics then
		variables.explosive_details = true
	end
