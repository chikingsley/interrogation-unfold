...
M.run = scenes.skippable(function ()
	...
	local unavailable_agents = {}

	if variables.novak_assignee then
		unavailable_agents[variables.novak_assignee] = true
	end

	if variables.adams_assignee then
		unavailable_agents[variables.adams_assignee] = true
	end
