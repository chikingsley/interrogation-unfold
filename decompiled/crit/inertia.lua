...
function Inertia.new(options)
	options = options or {}
	local lowpass_smoothen = filters.low_pass(options.smoothen_cutoff or 15)
	local lowpass_friction = filters.low_pass(options.friction_cutoff or 1)
	local epsilon = options.rest_epsilon or 1
	...
	local function update(dt, panned)
		...
		if not scalar and vmath.length_sqr(velocity) < epsilon_squared or scalar and math.abs(velocity) < epsilon then
			self.velocity = zero
