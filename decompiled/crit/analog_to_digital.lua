...
local input_map = {
	[h_gamepad_lstick_right] = {
		stick = 1,
		x = 1,
		action_id = h_gamepad_lstick_digital_right
	},
	[h_gamepad_lstick_left] = {
		stick = 1,
		x = -1,
		action_id = h_gamepad_lstick_digital_left
	},
	[h_gamepad_lstick_up] = {
		stick = 1,
		y = 1,
		action_id = h_gamepad_lstick_digital_up
	},
	[h_gamepad_lstick_down] = {
		stick = 1,
		y = -1,
		action_id = h_gamepad_lstick_digital_down
	},
	[h_gamepad_rstick_right] = {
		stick = 2,
		x = 1,
		action_id = h_gamepad_rstick_digital_right
	},
	[h_gamepad_rstick_left] = {
		stick = 2,
		x = -1,
		action_id = h_gamepad_rstick_digital_left
	},
	[h_gamepad_rstick_up] = {
		stick = 2,
		y = 1,
		action_id = h_gamepad_rstick_digital_up
	},
	[h_gamepad_rstick_down] = {
		stick = 2,
		y = -1,
		action_id = h_gamepad_rstick_digital_down
	},
	[h_gamepad_ltrigger] = {
		stick = 3,
		unidirectional = true,
		x = 1,
		action_id = h_gamepad_ltrigger_digital
	},
	[h_gamepad_rtrigger] = {
		stick = 4,
		unidirectional = true,
		x = 1,
		action_id = h_gamepad_rtrigger_digital
	},
	[h_gamepad_disconnected] = {
		disconnected = true
	}
}
local deadzone = 0.5
...
local function make_stick(stick_index, gamepad)
	local last_action_id, digital_action_id, pending_x, pending_y = nil
	local last_x = 0
	local last_y = 0
	local action_id_1, action_1, action_id_2, action_2 = nil
	...
	local function set_value(new_action_id, x, y)
		...
		if digital_action_id == new_digital_action_id then
			if new_digital_action_id then
				...
				if (did_repeat and repeat_interval or repeat_delay) <= timestamp - last_timestamp then
	...
	local function reset(action_id)
		...
		if action_id and digital_action_id then
	...
	local function update()
		if bidirectional then
			if (pending_x or pending_y) and (not pending_x or not pending_y) then
				bidirectional = false
			end
		elseif pending_x and pending_y then
			bidirectional = true
		end
...
local M = {
	set_input_map = function (new_input_map)
		input_map = new_input_map
		gamepads = {}
		stick_count = 0

		for k, def in pairs(input_map) do
			if stick_count < def.stick then
		...
		for k, def in pairs(input_map) do
			if def.x == 1 then
				stick_directions[def.stick].right = def.action_id
			elseif def.x == -1 then
				stick_directions[def.stick].left = def.action_id
			elseif def.y == 1 then
				stick_directions[def.stick].up = def.action_id
			elseif def.y == -1 then
				stick_directions[def.stick].down = def.action_id
			end
	...
	on_input = function (action_id, action)
		...
		local x = def.x
		local y = def.y
		x = x and x * action.value
		y = y and y * action.value
