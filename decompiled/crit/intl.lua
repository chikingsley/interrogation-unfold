...
local function warn(message, show_trace)
	if strict then
		error(message, 3)
	elseif show_trace and debug then
...
local function make_namespace(namespace_id, custom_loader)
	...
	local function get_entry(key)
		for i = 1, lang_count do
			...
			if entry then
				if warn_fallback and lang_fallback < i then
...
local namespace_privates = {
	[M] = M_private
}

setmetatable(namespace_privates, {
	__mode = "k"
})
...
function M.init(options)
	...
	warn_fallback = options and not not options.warn_fallback
	strict = options and not not options.strict
	uninitialized = false
