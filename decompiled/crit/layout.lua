...
Layout.design_gui_left = 0
Layout.design_gui_bottom = 0
Layout.design_gui_right = design_width
Layout.design_gui_top = design_height
Layout.design_go_left = 0
Layout.design_go_bottom = 0
Layout.design_go_right = design_width
Layout.design_go_top = design_height
local window_width, window_height, viewport_width, viewport_height, viewport_origin_x, viewport_origin_y, design_offset_x, design_offset_y, projection, gui_projection, projection_width, projection_height, projection_left, projection_right, projection_top, projection_bottom, viewport_to_projection_scale_x, viewport_to_projection_scale_y, projection_to_viewport_scale_x, projection_to_viewport_scale_y, design_to_window_scale_x, design_to_window_scale_y = nil
...
function Layout.set_metrics(metrics)
	...
	viewport_origin_x = metrics.viewport_origin_x or math.ceil((window_width - viewport_width) * (metrics.viewport_grav_x or 0.5))
	viewport_origin_y = metrics.viewport_origin_y or math.ceil((window_height - viewport_height) * (metrics.viewport_grav_y or 0.5))
...
function Layout.get_projection_matrix()
	if not projection then
		projection = vmath.matrix4_orthographic(projection_left, projection_right, projection_bottom, projection_top, -1, 1)
	end
...
function Layout.new(opts)
	...
	self.is_go = is_go
	self.get_metrics = opts and opts.get_metrics or is_go and get_go_metrics or get_gui_metrics
	local left, bottom, right, top = nil
	...
	if not opts or not opts.no_initial_place then
		msg.post("#", h_window_change_size, {
			width = 0,
			height = 0
		})
	end
...
local scale_func = {
	x = function (width, height, design_width_, design_height_, scale_x)
		return scale_x
	end,
	y = function (width, height, design_width_, design_height_, scale_x, scale_y)
		return scale_y
	end,
	fit = function (width, height, design_width_, design_height_, scale_x, scale_y)
		return math.min(scale_x, scale_y)
	end,
	cover = function (width, height, design_width_, design_height_, scale_x, scale_y)
...
function Layout.__index:add_node(node, opts)
	...
	local pivot = vmath.vector3(self.design_width * design_grav_x, self.design_height * design_grav_y, 0)
	local node_spec = {
		node = node,
...
function Layout.__index:place()
	...
	for i, node in ipairs(self.nodes) do
		...
		local pivot = vmath.vector3(width * grav_x, height * grav_y, 0)
