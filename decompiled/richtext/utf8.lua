...
function utf8unicode(str, i, j, byte_pos)
	...
	if bytes == 2 then
		...
		local code0 = byte0 - 192
		local code1 = byte1 - 128
		unicode = code0 * shift_6 + code1
	end

	if bytes == 3 then
		local byte0, byte1, byte2 = byte(ch, 1, 3)
		local code0 = byte0 - 224
		local code1 = byte1 - 128
		local code2 = byte2 - 128
		unicode = code0 * shift_12 + code1 * shift_6 + code2
	end

	if bytes == 4 then
		local byte0, byte1, byte2, byte3 = byte(ch, 1, 4)
		local code0 = byte0 - 240
		local code1 = byte1 - 128
		local code2 = byte2 - 128
		local code3 = byte3 - 128
...
local function matcherGenerator(regex, plain)
	...
	matcher.functions[1] = function (_)
		...
		if matcher.startStr < matcher.str and matcher.fromStart or matcher.stringLen <= matcher.str then
			matcher.stop = true
