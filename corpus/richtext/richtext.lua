...
local M = {
	ALIGN_CENTER = hash("ALIGN_CENTER"),
	ALIGN_LEFT = hash("ALIGN_LEFT"),
	ALIGN_RIGHT = hash("ALIGN_RIGHT")
}
...
local function get_font(word, fonts)
	...
	if font_settings then
		...
		font = font or font_settings.regular
	end
...
local function compare_words(one, two)
	...
	local one_tags = one.tags
	local two_tags = two.tags
...
local function measure_node(word, font, previous_word)
	local node, metrics, combined_metrics = nil
...
function M.create(text, font, settings)
	...
	settings.fonts[font] = settings.fonts[font] or {
		regular = hash(font)
	}
	...
	for i = 1, word_count do
		...
		local overflow = settings.width and settings.width < (combined_metrics and line_width - previous_word.metrics.total_width + combined_metrics.width or line_width + word_metrics.width)
...
function M.on_click(words, action)
	for i = 1, #words do
		...
		if word.anchor and gui.pick_node(word.node, action.x, action.y) and word.tags and word.tags.a then
			local message = {
	...
	if options and options.words then
		...
		for i = 1, #words do
			...
			if count < length and is_text_node then
				...
				if length < count + word_length then
