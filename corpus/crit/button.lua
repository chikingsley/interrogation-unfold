...
local function default_action_to_position(action)
...
local Button_default_on_pass_focus = nil
...
function Button.new(node, self)
	...
	self.action_to_position = self.action_to_position or is_sprite and Button.default_sprite_action_to_position or Button.default_gui_action_to_position
	self.pick = self.pick or is_sprite and Button.default_sprite_pick or Button.default_gui_pick
	...
	if self.focus_simulates_hover then
		function self.on_state_change(button, button_state, old_state, did_click)
		...
		function self.on_focus_change(button, focused)
			if focused then
...
local function Button_mapped_action_id_to_navigation_action(mapped_action_id)
	local is_gamepad = GAMEPAD <= mapped_action_id
			...
			if action.released then
				...
				if self.mouse_down then
					...
					if mouse_can_press or self.hover_from_external_touch then
						local new_state = is_hovering and (self.mouse_can_press and self.action and STATE_PRESSED or STATE_HOVER) or STATE_DEFAULT
...
function Button:default_sprite_pick(action)
...
function Button:default_gui_pick(action)
	local node = self.node
...
function Button:focus_ring_on_focus_change(focused)
	...
	if self.is_sprite then
		if focused then
			...
			go.animate(node, h_tintw, go.PLAYBACK_ONCE_FORWARD, 1, go.EASING_LINEAR, focus_duration)
		else
			go.cancel_animations(node, h_tintw)
			go.animate(node, h_tintw, go.PLAYBACK_ONCE_FORWARD, 0, go.EASING_LINEAR, focus_duration, 0, function ()
	...
	elseif focused then
		...
		gui.animate(node, h_colorw, 1, go.EASING_LINEAR, focus_duration)
	else
		gui.cancel_animation(node, h_colorw)
		gui.animate(node, h_colorw, 0, go.EASING_LINEAR, focus_duration, 0, function ()
			gui.set_enabled(node, false)
		end)
	end
end

function Button:fade_on_state_change(state)
	local value = nil
...
function Button:darken_on_state_change(state)
	local value = nil
...
function Button:flipbook_on_state_change(state)
	local animation = nil
