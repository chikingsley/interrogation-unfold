...
local filter = filters.low_pass(3)
local filter_vel = filters.low_pass(1)
local Scroll = {
	__index = {},
...
function Scroll.new(opts)
	...
	local self = {
		...
		action_to_dy = opts.action_to_dy or opts.is_go and Scroll.default_go_action_to_dy or Scroll.default_gui_action_to_dy
	}
...
function Scroll.__index:acquire_control(sender, interrupt_callback)
	...
	if self.sender and (not old_interrupt_cb or not old_interrupt_cb(sender)) then
		return false
	end
...
function Scroll.__index:add_node(node, position)
	position = position or self.is_go and go.get_position(node) or gui.get_position(node)
	local index = table.insert(self.nodes, {
		node,
		position
	})
...
function Scroll.__index:animate_offset(offset, duration, easing)
	local function interrupt_callback()
		self.animation = nil
...
function Scroll.__index:nearest_offset_covering_range(top, bottom)
	...
	if bottom > offset + view_height then
...
function Scroll.__index:update(dt)
	...
	if not self.sender then
		...
		if offset < 0 or max_offset < offset then
...
function Scroll.__index:on_input(action_id, action)
	...
	elseif not action_id and self.sender == h_touch then
		return true
	end
end
