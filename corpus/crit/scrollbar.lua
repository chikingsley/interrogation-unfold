...
function ScrollBar.default_gui_action_to_position(action)
	return action.x, action.y
end

ScrollBar.click_action_id = hash("click")
...
local function ScrollBar_on_offset_change(self)
	...
	if view_height < content_height then
	...
	if action_id == ScrollBar.click_action_id then
		if action.pressed then
			...
			if gui.pick_node(self.node, self.action_to_position(action)) then
				...
				if dx < -half_width or half_width < dx then
					return
				end
		...
		elseif self.grab then
			...
			local current_grab = self._invert_if_horizontal((gui.get_screen_position(self.node) - action_to_vec3(action))[self.axis])
